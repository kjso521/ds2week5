...
from code_denoising.common.logger import logger, logger_add_handler, init_logger
...
class Trainer:
    ...
    def __init__(self) -> None:
        ...
        if ModelType.from_string(self.config.model_type) == ModelType.Unet:
            self.config.model_config = self.config.unet_config
        elif ModelType.from_string(self.config.model_type) == ModelType.DnCNN:
            self.config.model_config = self.config.dncnn_config

        self.run_dir = Path(self.config.run_dir) / f"{self.config.tag}"
